  ...
  private class Instance extends GeneralizedOPTICS.Instance<V, CorrelationClusterOrder> {
    ...
    protected void expandDBID(DBIDRef id) {
      ...
      for(DBIDIter iter = relation.iterDBIDs(); iter.valid(); iter.advance()) {
		...
		System.out.println("main HISC loop, id1 = " + id + " ; id2 = " +  iter);
		
        long[] pv2 = index.getPreferenceVector(iter);
        V v2 = relation.get(iter);
        final long[] commonPreferenceVector = BitsUtil.andCMin(pv1, pv2); // BITWISE AND OR!!!
		...
		System.out.println("v1: " + v1 + " ; v2: " + v2 + " ; pv1 = " + BitsUtil.toStringLow(pv1, dim) + " ; pv2 = " + BitsUtil.toStringLow(pv2, dim));

        if(dist1 > alpha || dist2 > alpha) {
          subspaceDim++;
          if(LOG.isDebugging()) {
            LOG.debugFine(new StringBuilder(100) //
                .append("weighted dist1: ").append(dist1) //
                .append("\nweighted dist2: ").append(dist2) //
        ...
        if(prevdim < subspaceDim) {
			System.out.println("prevdim: " + prevdim + " ; current subspaceDim: " + subspaceDim + " : cannot be better, ignore & continue");
          continue;
        }
		else{
			System.out.println("prevdim: " + prevdim + " ; current subspaceDim: " + subspaceDim);
		}

        // Note: used to be: in orthogonal subspace?
        double orthogonalDistance = weightedDistance(v1, v2, commonPreferenceVector);

		System.out.println("orthogonal distance with common preference vector: " + orthogonalDistance);
	

        if(prevdim == subspaceDim) {
          double prevdist = reachability.doubleValue(iter);
          if(prevdist <= orthogonalDistance) {
			System.out.println("prevdist <= orthogonalDistance, ignore & continue");
            continue; // Not better.
          }
        }
		...
		System.out.println("set predecessor " + iter + " --> " + id);
