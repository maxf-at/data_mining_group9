...
public class HiSCPreferenceVectorIndex<V extends NumberVector> extends AbstractPreferenceVectorIndex<V> implements PreferenceVectorIndex<V> {
  ...
  public void initialize() {
    ...
    FiniteProgress progress = LOG.isVerbose() ? new FiniteProgress("\nPreprocessing preference vector", relation.size(), LOG) : null;
    long start = System.currentTimeMillis();
    for(DBIDIter it = relation.iterDBIDs(); it.valid(); it.advance()) {
      storage.put(it, determinePreferenceVector(relation, it, knnQuery.getKNNForDBID(it, k)));
	  
	  //System.out.println("\nk nearest neighbor: " + knnQuery.getKNNForDBID(it, k));
  ...
  private long[] determinePreferenceVector(Relation<V> relation, DBIDRef id, DBIDs neighborIDs) {
	...
	System.out.println("\nVariances for id " + id + " / neighbors:" + neighborIDs);
    ...
    for(DBIDIter iter = neighborIDs.iter(); iter.valid(); iter.advance()) {
      ...
      for(int d = 0; d < dim; d++) {
        ...
        sumsq[d] += diff * diff;
		//System.out.println("sumsq[" + d + "]=" + sumsq[d] + " / o = " + o.doubleValue(d) + " / p = " + p.doubleValue(d));
		System.out.println("(" + o.doubleValue(d) + " - " + p.doubleValue(d) + ")^2 = sumsq[" + d + "] = " + sumsq[d]);
      }
    }

    // preference vector
	System.out.println("\nPreference vector for id " + id);
    ...
    for(int d = 0; d < dim; d++) {
	  ...
	  System.out.println("sumsq[" + d + "]=" + sumsq[d] + " ; alpha=" + alpha + " ; k = " + size + " ; comparison: " + logic);
      if(sumsq[d] < alpha * size) {
		//System.out.println("sumsq[d] < alpha * size");
